...
#THIS RUNS AS A NORMAL, NON-ARRAY JOB FOR A RANDOM SUBSET OF THE PARKS DATA 
...
sites_big = gpd.read_file("../Parks/centroids.shp")
random.seed(42)
...
def comp_all_mats(sites, 
    ...
    np.savez("test_parks_sample_geod", walk_speed = walk_speed, dists_geod = dists_geod, time_geod = time_geod)
    ...
    np.savez("test_parks_sample_walk", walk_speed = walk_speed, dists_walk = dists_walk, time_walk = time_walk)
    ...
    np.savez("test_parks_sample_drive", dists_drive = dists_drive, time_drive = time_drive)
